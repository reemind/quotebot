...
namespace QuotePanel
{
    public class VkCallbackApi
    {
        ...
        public void MessageNew(JsonElement vkobj)
        {
            ...
            var user = context.Users.SingleOrDefault(t => t.VkId == id);
            var role = user is Data.User?context.GetGroupRole(group, user): null;
            ...
            {
                Response(Resources.Resource.Help + (context.InRole(user, group, UserRole.Admin) ? Resources.Resource.HelpAdmin : ""));
            }
            else if (context.InRole(user, group, UserRole.Admin))
            {
                if (Check(@"\A[Зз]аадминить https:\/\/vk\.com\/([\S]+)"))
                {
                    ...
                    if (api_user is VkNet.Model.User)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                context.SetRole(group, user, UserRole.Moder);
        ...
        public void WallReplyNew(JsonElement vkobj)
        {
            ...
            vkobj.EnumerateObject().Count(t => t.Name == "reply_to_comment") == 0)
            {
                ...
                using (var scope = context.Database.BeginTransaction())
                {
                    ...
                    var quote = quotes.FirstOrDefault(t => t.User == user);
                    if (quote == null)
                    {
                        count++;
        ...
        public void WallNewPost(JsonElement vkobj)
        {
            ...
            {
                var role = context.GetGroupRoles(group).Include(t => t.User)
                              .FirstOrDefault(t =>
                                    t.User.VkId == el.GetInt64() && t.Role > UserRole.User);
                if (role != null)
                {
                    if (context.GetPosts(group).FirstOrDefault(t => t.PostId == vkobj.GetProperty("id").GetInt32()) != null)
                    {
                        SendMessage(role.User, "Запись уже добавлена");
                        return;
                    }

                    var new_post = CreatePost(vkobj, max);

                    SendMessage(role.User, $"Запись добавлена в очередь: id={new_post.Id}", new MessagePost(group.GroupId, new_post.PostId));
                    //api.Wall.CreateComment(new VkNet.Model.RequestParams.WallCreateCommentParams
                    //{
                    //    OwnerId = -group.GroupId,
                    //    PostId = new_post.PostId,
